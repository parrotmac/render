...
pub fn suspend(token: &str, args: &ArgMatches) -> anyhow::Result<()> {
    ...
    let available_services = runtime.block_on(async { api::list_services(token).await })?;
    ...
    let services = services
        .map(|s| {
            Ok::<_, anyhow::Error>((
                ...
                available_services
                    .get(s)
                    .ok_or_else(|| anyhow!("Service {} not found.", s))?
                    .as_str(),
            ))
        })
    ...
    for (name, id) in services {
        runtime.block_on(async {
            api::suspend(token, id)
                .await
                .map(|_env_vars| println!("Suspended service: {}", name))
                .map_err(|e| {
