...
fn main() -> anyhow::Result<()> {
    ...
    match args.subcommand().unwrap() {
        ("put-env", args) => command::put_env::put_env(token, args),
        ("get-env", args) => command::put_env::get_env(token, args),
        ("deploy", args) => command::deploy::deploy(token, args),
        ("list", args) => command::list::list_services(token),
        ("suspend", args) => command::suspend::suspend(token, args),
        _ => unreachable!(),
    }
}
